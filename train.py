...
parser.add_argument('--wandb_id', type=str, default="starain", help='weight & bias id')
args = parser.parse_args()
os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu

if len(args.wandb_id) != 0:
    wandb.init(
        project="Multimodal Sentiment Analysis using " + args.text_model + " and " + args.image_model,
        config=args,
...
def evaluate(model, valid_dataloader):
    ...
    val_true, val_pred = [], []
    with torch.no_grad():
        for batch, (images, texts, labels) in enumerate(valid_dataloader):
            images = images.cuda()
    ...
    # print("pred:{},label:{}".format(val_pred,val_true))
...
def train_and_eval(model, train_dataloader, valid_dataloader, optimizer,scheduler):
    ...
    for i in range(args.epochs):
        ...
        total_loss = 0.0
        for batch, (images, texts, labels) in enumerate(train_dataloader):
            images = images.cuda()
            texts = {name: tensor.squeeze(1).cuda() for name, tensor in texts.items()}
            labels = labels.cuda()
            outputs = model(texts,images)
...
def main():
    ...
    data_loader=MultimodalDataloader(args.train_data,
                                     args.data_folder,
